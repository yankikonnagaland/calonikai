...
import { Camera, Upload, X, Zap, Loader2, CheckCircle, AlertCircle, Plus } from "lucide-react";
...
export default function FoodCamera({ sessionId, selectedDate, onFoodDetected, onMealItemAdded }: FoodCameraProps) {
  ...
  const addMealMutation = useMutation({
    mutationFn: async (data: { foodId: number; quantity: number; unit: string; sessionId: string; date?: string }) => {
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/meal/${sessionId}/${dateParam}`] });
      queryClient.invalidateQueries({ queryKey: [`/api/meal/${sessionId}`] });
      queryClient.removeQueries({ queryKey: [`/api/meal/${sessionId}/${dateParam}`] });
      queryClient.refetchQueries({ queryKey: [`/api/meal/${sessionId}/${dateParam}`] });
    },
    onError: (error) => {
        ...
        img.onload = () => {
          ...
          if (previewContext) {
            ...
            previewContext.drawImage(
              img, 
              cropX, cropY, sourceSize, sourceSize,  // source crop
              0, 0, previewSize, previewSize         // destination
            );
          ...
          toast({
            ...
            description: "Image optimized for analysis. Click 'Analyze Food' to identify nutrients",
          });
        };
        img.onerror = () => {
      ...
      const analyzeImage = new Promise<void>((resolve, reject) => {
        img.onload = async () => {
          try {
            ...
            console.log("Optimized image data:", optimizedImageData.substring(0, 50) + "...");
            ...
            console.log("Sending optimized base64 data length:", base64Data.length);
            
            // Use admin session ID if in admin mode
            const isAdminMode = localStorage.getItem('admin_mode') === 'true';
            const effectiveSessionId = isAdminMode ? localStorage.getItem('session_id') || sessionId : sessionId;
      ...
      if (error.message && error.message.includes("Daily limit reached")) {
        ...
        if (isPremium) {
          toast({
            ...
            description: "You've used all 5 daily photo analyses. Your limit resets tomorrow.",
            variant: "destructive",
          });
        } else {
  ...
  const addSingleFoodToMeal = async (detectedFood: any) => {
    try {
      ...
      const baseHash = Math.abs(foodString.split('').reduce((a, b) => {
      ...
      return new Promise((resolve, reject) => {
        addMealMutation.mutate({
          ...
          onError: (error) => {
            console.error(`Failed to add ${detectedFood.name} to meal:`, error);
            reject(error);
          }
        });
      });
        ...
        try {
          ...
          await new Promise((resolve, reject) => {
            addMealMutation.mutate({
              ...
              onSuccess: (data) => {
                console.log(`Successfully added ${detectedFood.name} to meal with ID:`, data);
  ...
  const remainingScans = usageStats?.remaining?.photos !== undefined ? usageStats.remaining.photos : Math.max(0, photoLimit - photosUsed);
      ...
      <Card className="bg-gradient-to-br from-purple-50 to-blue-50 dark:from-purple-950/20 dark:to-blue-950/20 border-purple-200 dark:border-purple-800">
        <CardHeader>
          <CardTitle className="flex items-center text-purple-800 dark:text-purple-200">
            ...
            AI Food Camera
            {isAuthenticated && (
              <Badge variant="outline" className="ml-auto text-xs border-purple-300 bg-[#1b2027]">
          ...
          {isLimitReached && isAuthenticated && (
            <div className="text-sm text-orange-600 bg-orange-50 p-2 rounded-md border border-orange-200 mt-2 flex items-center justify-between">
              <span>
                Daily Limit Reached. {isPremium ? "Try again tomorrow." : "Upgrade to premium for more scans."}
              </span>
              {!isPremium && (
                <Button
        ...
        {!capturedImage && (
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Take a photo or upload an image to instantly identify food and get nutrition information.
            </p>
        ...
        {isOpen && cameraMode === 'camera' && !capturedImage && (
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              >
                <Camera className="w-5 h-5 mr-2" />
                {cameraReady ? "📸 Capture Food Photo" : "Initializing Camera..."}
              </Button>
              <Button 
                onClick={stopCamera} 
                variant="outline"
              ...
              <div className="space-y-4 border-t pt-4">
                        ...
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                              Confidence: {Math.min(100, Math.round(food.confidence * 100))}%
                            </p>
                          </div>
                        </div>
                  ...
                  <div className="bg-orange-50 dark:bg-orange-950/20 p-3 rounded-lg border border-orange-200 dark:border-orange-800">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-sm text-orange-700 dark:text-orange-300">
                        No food items detected. Try taking a clearer photo or uploading a different image.
                      </p>
                    </div>
                  </div>
                )}
                ...
                {analysisResult.suggestions && analysisResult.suggestions.length > 0 && (
                  <div className="bg-blue-50 dark:bg-blue-950/20 p-3 rounded-lg border border-blue-200 dark:border-blue-800">
                    <h4 className="font-medium text-blue-700 dark:text-blue-300 mb-2">AI Suggestions:</h4>
